# -*- coding: utf-8 -*-
"""
Created on Sat Nov 27 12:59:17 2021
...
enh_valueCounts = enhs_status.apply(pd.value_counts) / len(enhs)
enh_valueCounts = enh_valueCounts.sort_index()
...
for ct in cell_types:
    ...
    for i in [0,1,2,3]:
        frac = enh_valueCounts.at[i,"%s_status" % ct]
        if i == 2:
            n_acClosed[ct]= round(frac*len(enhs))
        ax1.text(xs[i],frac+.02,"%.1f" % (frac*100),rotation = 90, ha = 'center', fontsize = 6)
    xbase = xbase + 5
...
fig.legend(handles, labels, fontsize = 6, handlelength= 0.5)
...
total = len(enhs)
for ct1,ct2 in [('MPP','CMP'),('CMP','GMP'),('GMP','Mono'),('GMP','Gran'),('MPP','Bcell')]:
    ac_closed1 = n_acClosed.get(ct1)
    ac_closed2 = n_acClosed.get(ct2)
    not_acclosed1 = total - ac_closed1
    not_acclosed2 = total - ac_closed2
    print(ct1,ct2,stats.fisher_exact([[ac_closed1,ac_closed2],
                                      [not_acclosed1,not_acclosed2]], 
                                     alternative = 'greater'))
...
for ct in cell_types:
    ...
    frac = enh_valueCounts.at[2,"%s_status" % ct]
    ax2.text(xs[2],frac+.02,"%.1f" % (frac*100),rotation = 90, ha = 'center', fontsize = 6)
    frac = enh_valueCounts.at[1,"%s_status" % ct]
    ax2.text(xs[1],frac+.02,"%.1f" % (frac*100),rotation = 90, ha = 'center', fontsize = 6)
    xbase = xbase + 5
...
plt.tight_layout()
plt.savefig('C:/Users/andym/OneDrive/Documents/MICB405/figures/enhancer_status_fraction_byCellType.pdf')
plt.savefig('C:/Users/andym/OneDrive/Documents/MICB405/figures/enhancer_status_fraction_byCellType.png')